...
def stub_reply(state: AgentState) -> AgentState:
    ...
    state.draft_v1 = "Bien reçu, merci. Je reviens vers vous rapidement avec une réponse complète."
...
def stub_ask_clarification(state: AgentState) -> AgentState:
    ...
    state.draft_v1 = "Pour que je traite correctement : quel est exactement le besoin et l’échéance attendue ?"
...
def stub_escalate(state: AgentState) -> AgentState:
    ...
    state.actions.append({
        "type": "handoff_human",
        "summary": "Demande à risque / sensible ou nécessitant validation humaine avant réponse.",
    })
...
def stub_ignore(state: AgentState) -> AgentState:
    ...
    state.actions.append({
        "type": "ignore",
        "reason": "Hors périmètre / spam / pas d’action requise.",
    })
